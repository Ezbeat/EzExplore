...
)
{
    ...
    findHandle = FindFirstFileA(exploreDirectoryPath_.c_str(), &findData);
    if (findHandle == INVALID_HANDLE_VALUE)
    {
        return retValue;
    }
    ...
    do
    {
        if ((findData.cFileName[0] == '.' && findData.cFileName[1] == NULL) ||
            (findData.cFileName[0] == '.' && findData.cFileName[1] == '.' && findData.cFileName[2] == NULL))
        {
            continue;
        }
...
)
{
    ...
    auto exploreCallbackLambda = [](const FileInfoW& fileInfo, /*_In_opt_*/ void* userContext)->Errors
    {
...
)
{
    ...
    auto exploreCallbackLambda = [](const FileInfoA& fileInfo, /*_In_opt_*/ void* userContext)->Errors
    {
        if (fileInfo.isDirectory == true)
        {
            (static_cast<uint32_t*>(userContext))[1]++;
